...
class Star implements PrinterInterface {
  ...
  // ------------------------------ Beep ------------------------------
  beep() {
    console.error("Beep not supported on Star!");
    return new Buffer("");
  }

  // ------------------------------ Set text size ------------------------------
  setTextSize() {
    console.error("setTextSize not supported on Star!");
    return new Buffer("");
  }

  // ------------------------------ Append ------------------------------
  append(appendBuffer: Buffer) {
    if (this.buffer) {
  ...
  printQR(data: string, settings: any) {
    ...
    const models: Record<number, Buffer> = {
      1: this.config.QRCODE_MODEL1,
      2: this.config.QRCODE_MODEL2
    };

    const correctionLevels: Record<string, Buffer> = {
    ...
    const cellSizes: Record<number, Buffer> = {
    ...
    //	• When using this command, the printer receives data for the number of bytes (k) specified by nL and nH. The data automatically
    //    expands to be set as the qr code data.
    //	• Indicates the number bytes of data specified by the nL and nH. Bar code data is cleared at this time.
    //	• The data storage region of this command is shared with the manual setting command so data is updated each time either command
    //    is executed.
    const s = data.length;
    const lsb = parseInt(String(s % 256));
    const msb = parseInt(String(s / 256));
    ...
    // When receiving this command, if there is unprinted data in the image buffer, the printer will print the bar code after printing the
    // unprinted print data.
    // A margin of more than 4 cells is required around the QR code. The user should ensure that space. Always check printed bar codes
    // in actual use.
  ...
  maxiCode(data: string, settings: any) {
    console.error("maxiCode not supported on Star!");
    return new Buffer("");
  }
